...
public class test extends ControlSystemMFN150
{
    ...
    public int takeStep()
    {
        ...
        //System.out.println("takeStep " + pb);

        if (pb != null)
        {
            //check for button pushes
            but = pb.getlastbutton();
            if (but.equals("reset"))
            {
                abstract_robot.resetPosition(new Vec2(0,0));
                but = "stop";
            }
            ...
            noisegain = pb.getnoisegain();
            if (noisegain != oldnoisegain) configure();
            oldnoisegain = noisegain;

            avoidgain = pb.getavoidgain();
            if (avoidgain != oldavoidgain) configure();
            oldavoidgain = avoidgain;
        }

        // STEER
        result = steering_configuration.Value(curr_time);
        if (but.equals("stop")) result.setr(0);
    ...
    public void trialInit()
    {
        ...
        // kill off old ParamBox if it exists
        if (pb != null)
            pb.dispose();

        // open a new ParamBox
        pb = new ParamBox();
        String but = pb.getlastbutton();
        // System.out.println("trialInit2");
